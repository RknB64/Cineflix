...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : lun. 05 fév. 2024 à 19:16
...
CREATE TABLE IF NOT EXISTS `achat` (
  ...
  `nb_places_achetées` int(10) NOT NULL,
...
INSERT INTO `achat` (`id_billet`, `horaire_date`, `date_d’achat`, `nb_places_achetées`, `id_Adherent`, `id_seance`) VALUES
...
INSERT INTO `adherent` (`id`, `nom`, `prenom`, `mail`, `password`, `ponts`, `date_de_creation`, `compte`, `id_ville`) VALUES
...
CREATE TABLE IF NOT EXISTS `film` (
  ...
  `durée` varchar(5) NOT NULL,
  `État` enum('cinéma','streaming') NOT NULL,
  `id_affiche` int(20) NOT NULL,
  `date_de_sortie` timestamp NOT NULL,
  `date d’exploitation` timestamp NOT NULL,
  PRIMARY KEY (`Id`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8;
...
INSERT INTO `film` (`Id`, `titre`, `description`, `durée`, `État`, `id_affiche`, `date_de_sortie`, `date d’exploitation`) VALUES
(1, 'Fast and Furious ', 'action ', '1H50', 'cinéma', 1, '2024-02-14 18:44:26', '2024-02-22 18:44:26');
...
CREATE TABLE IF NOT EXISTS `stream` (
  ...
  `date d’expiration` timestamp(6) NOT NULL,
  `id_film` int(20) NOT NULL,
  `id_adherent` int(20) NOT NULL,
...
INSERT INTO `stream` (`date_achat`, `date d’expiration`, `id_film`, `id_adherent`) VALUES
...
--
ALTER TABLE `achat`
  ADD CONSTRAINT `fk_achat_adhernet` FOREIGN KEY (`id_Adherent`) REFERENCES `adherent` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
